...
(function($) {
	...
	$.fn.formly = function(options, callback) {
		...
		this
				.append('<div style="clear:both;"></div><div class="formlyAlerts"></div>');
		...
		this.children().each(function(index, item) {
			...
			if ($(item).attr('place')) {
				if ($(item).attr('type') == 'password') {
					...
					$(item).after('<input type="text" id="' + hID + '" value="'
							+ $(item).attr('place')
							+ '" class="formlyPWPlaces" />');
					$('#' + hID).css('color', '#bbb');
					$(item).hide();
					$('#' + hID).show();

					// Focus and blur must be handled independently for
					// variables sake
					$('#' + hID).focus(function() {
								$('#' + hID).hide();
								$(item).show();
			...
			$(item).blur(function() {
				...
				if ($(item).attr('pre-fix')) {
					...
					if (thePrefix.length == 1) {
						if (originalVal.charAt(0) != thePrefix
								&& $(item).val() != $(item).attr('place')) {
							$(item).val(thePrefix + originalVal);
						}
					} else {
						if (originalVal.indexOf(thePrefix) == -1
								&& $(item).val() != $(item).attr('place')) {
							$(item).val(thePrefix + originalVal);
						}
					}
			...
			$('#' + formName).find('input:reset').click(function(item) {
				item.preventDefault();
				$('#' + formName)
						.find('input:text, input:password, input:checkbox, input:radio')
						.each(function() {
		...
		this.submit(function(item) {
					...
					if (!canSubmit) {
						...
						if (callback) { // Change this to .serializeArray() for
										// JSON
							item.preventDefault();
		...
		var functions = {
			...
			validate : function(item) {
				...
				if (!valid) {
					if (!$('#' + alertName).is(':visible')) {
						$('#' + formName)
								.find('.formlyAlerts')
								.append('<div class="formlyInvalid formlyAlert" id="'
										+ alertName
										+ '">Invalid '
										+ validType
										+ '</div>')
					...
					var borderColor = $('#' + alertName)
							.css('background-color');
					$(item).css('border-color', borderColor);
					if ($(item).attr('type') == 'password') {
						$(item).next('.formlyPWPlaces').css('border-color',
								borderColor);
					}
					return false;
				} else {
			...
			require : function(item) {
				...
				if (!$(item).val() || $(item).val() == $(item).attr('place')) {
					if (!$('#' + alertName).is(':visible')) {
						$('#' + formName)
								.find('.formlyAlerts')
								.append('<div class="formlyRequired formlyAlert" id="'
										+ alertName
										+ '">'
										+ label
										+ 'Required</div>')
			...
			match : function(item) {
				...
				if ($(item).val() != $('#' + formName).find('input[name='
						+ toMatch + ']').val()
						|| !$(item).val()) {
					if (!$('#' + alertName).is(':visible')) {
						$('#' + formName)
