		...
		tablesorter : new function() {
			...
			function detectParserForColumn(table, rows, rowIndex, cellIndex) {
				...
				while (nodeValue == '' && keepLooking) {
					rowIndex++;
					if (rows[rowIndex]) {
						node = getNodeFromRowAndCellIndex(rows, rowIndex,
								cellIndex);
			...
			function buildCache(table) {
				...
				for (var i = 0; i < totalRows; ++i) {
					...
					// if this is a child row, add it to the last row's children
					// and
					...
					cols.push(cache.normalized.length); // add position for
														// rowCache
			...
			function multisort(table, sortList, cache) {
				...
				for (var i = 0; i < l; i++) {
					...
					var s = (table.config.parsers[c].type == "text")
							...
							: ((order == 0) ? makeSortFunction("numeric",
									"asc", c) : makeSortFunction("numeric",
									"desc", c));
					var e = "e" + i;
			...
			function makeSortFunction(type, direction, index) {
				...
				if (type == 'text' && direction == 'asc') {
					...
					return "(" + a + " === null && " + b + " === null) ? 0 :("
							...
							+ b + " === null ? Number.NEGATIVE_INFINITY : " + a
							+ " - " + b + "));";
				} else if (type == 'numeric' && direction == 'desc') {
					return "(" + a + " === null && " + b + " === null) ? 0 :("
							+ a + " === null ? Number.POSITIVE_INFINITY : ("
							+ b + " === null ? Number.NEGATIVE_INFINITY : " + b
							+ " - " + a + "));";
				}
			};
			...
			this.construct = function(settings) {
				return this.each(function() {
					...
					$this.bind("update", function() {
						...
						var pos = [(cell.parentNode.rowIndex - 1),
								cell.cellIndex];
						// update cache
				...
				format : function(table) {
					...
					$("tr:visible", table.tBodies[0]).each(function(i) {
						...
						if (!$tr.hasClass(table.config.cssChildRow))
							row++;
						odd = (row % 2 == 0);
						$tr.removeClass(table.config.widgetZebra.css[odd
										? 0
										: 1])
								.addClass(table.config.widgetZebra.css[odd
												? 1
												: 0])
					});
					if (table.config.debug) {
