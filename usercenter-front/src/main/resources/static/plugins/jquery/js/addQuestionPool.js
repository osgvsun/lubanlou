...
function questionType(){
    ...
    {   $("#type").val(4);
        ...
        if(vs== "判断题"){
            $("#option_box").empty();
            $(".true_false_choice").append("<div class='true_false_choice'><table><tbody><tr><td><div class='true_false_choice_box'>对<input type='radio' name='true' value='对' title='对'/></div></td><td><div id='div12'></div></td></tr><tr><td><div class='true_false_choice_box'>错<input type='radio' name='true' value='错' title='错'/></div></td><td><div id='div13' style='margin-top:10px;'></div></td></tr></tbody></table><div class='customInput'><button type='button' onclick=\"window.history.go(-1)\" class='layui-btn layui-btn-primary' style='background: #f7f7f7 !important;line-height:32px !important;'>取消</button><button type='button' onclick=\"select()\" class='layui-btn layui-btn-normal'>确定</button></div></div>");
        }
        else if(vs== "填空题"){
            $("#type").val(8);
            $("#option_box").empty();
            $(".gap_filling").append("<div class='gap_filling'><h2>设置答案说明：</h2><p>填空题答案用大括号“{}”标记。<br/>例如：Roses are {red} and violets are {blue}. 则正确答案为“red”和“blue”。<br/>使用“|”分隔同义词。<br/>例如： {They are|They're} very happy. 则正确答案为“They are”或“They are”。<br/>使用星号(*)来表示通配符。<br/>例如： It's raining {c*} and {d*s}. 则形如“c...”和“d...s”的答案均为正确答案。</p><div class='customInput'><button onclick=\"window.history.go(-1)\" class='layui-btn layui-btn-primary' style='background: #f7f7f7 !important;line-height:32px !important;'>取消</button><button type='button' onclick=\"select()\" class='layui-btn layui-btn-normal'>确定</button></div></div>");
        }
        else if(vs== "简答题"){
            $("#type").val(5);
            $("#option_box").empty();
            $(".short_answer_question").append("<div class='short_answer_question'><table><tbody><tr><td>参考答案：</td><td><textarea name='shortanswer' id='shortanswer'></textarea></td></tr></tbody></table><h2>设置答案</h2><p>简答题答案用符号&&隔开。<br/>例如： 名字&&爱好&&生日</p><div class='customInput'><button type='button' onclick=\"window.history.go(-1)\" class='layui-btn layui-btn-primary' style='background: #f7f7f7 !important;line-height:32px !important;'>取消</button><button type='button' onclick=\"select()\" class='layui-btn layui-btn-normal'>确定</button></div></div>");
        }else if(vs== "请选择类型"){
            $("#option_box").empty();
        }
    }
...
function show_sub(option){
    ...
    $(".choice_box").append("<div class='customInput'><button type='button' onclick=\"window.history.go(-1)\" class='layui-btn layui-btn-primary' style='background: #f7f7f7 !important;line-height:32px !important;'>取消</button><button type='button' onclick=\"select()\" class='layui-btn layui-btn-normal'>确定</button></div>")
...
function select(){
    ...
    $.ajax({
        ...
        success: function(res) {
            if(res=="success") {
                alert("保存成功");
                // window.location.href="../questionPool/questionPoolContent?username="+username+"&questionPoolId="+questionPoolId+"&uid="+uid;
