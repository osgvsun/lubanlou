layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function() {
	...
	form.render(null, 'authorityclasscard');

	//执行表单
	table.render({
		elem: '#authorityclasscardtab',
		// url: layui.setter.base + "json/authorityClassCard.json", //数据接口
		url: iotHost + "/api/userAgentAuth/listAuthClassCard", //数据接口
		title: '表单',
		cellMinWidth: 130,
	...
	var $ = layui.$,
		active = {
			reload: function() {
				...
				//执行重载
				table.reload('authorityclasscardtab', {
					page: {
						curr: 1 //重新从第 1 页开始
					},
					where: {
