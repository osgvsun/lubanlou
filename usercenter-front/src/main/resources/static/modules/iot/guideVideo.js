layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function() {
    ...
    table.render({
        ...
        // url: iotHost + "/api/content/ListContentVideo", //数据接口
        title: '表单',
        cellMinWidth: 130,
    ...
    //监听行工具事件
    table.on('tool(guidevideotab)', function(obj) {
        ...
        if(obj.event === 'edit') {
            var index = layer.open({
                ...
                shade: 0.5,
                maxmin: true,
                content: 'guideEditVideo?id='+obj.data.id,
                zIndex: layer.zIndex //重点1
                ,
                success: function(layero) {
                    layer.setTop(layero); //重点2
