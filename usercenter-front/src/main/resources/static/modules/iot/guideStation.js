layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function() {
    ...
    //执行表单
    table.render({
        elem: '#guidestationtab',
        // url: layui.setter.base + "json/iotStationInstrument.json", //数据接口
        // url: iotHost + "/api/content/ListContentStation", //数据接口
        title: '表单',
        cellMinWidth: 130,
    ...
    //监听行工具事件
    table.on('tool(guidestationtab)', function(obj) {
        ...
        if(obj.event === 'edit') {
            var index = layer.open({
                ...
                shade: 0.5,
                maxmin: true,
                content: 'guideEditStation?id='+obj.data.id,
                zIndex: layer.zIndex //重点1
                ,
                success: function(layero) {
                    layer.setTop(layero); //重点2
