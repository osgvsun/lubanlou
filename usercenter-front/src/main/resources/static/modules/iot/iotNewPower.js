layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function() {
	...
	//监听提交
	form.on('submit(newpowerbtn)', function (data) {
		...
		$.ajax({
			url: iotHost + '/api/agent/addIotPower/',
			type: 'post',
			data: {
				hardwareIp,
				sn,
			...
			parent.layui.table.reload('iotpowercontrollertab'); //重载表格

	});

	form.render(null, 'newpower');
