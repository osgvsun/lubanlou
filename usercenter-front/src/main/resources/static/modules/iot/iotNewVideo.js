layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function () {
	...
	//监听提交
	form.on('submit(newvideobtn)', function (data) {
		...
		$.ajax({
			url: iotHost + '/api/agent/addIotVideo/',
			type: 'post',
			data: {
				hardwareIp,
				sn,
		...
		parent.layui.table.reload('iotvideotab'); //重载表格

	});

	form.render(null, 'newvideo');
	//获取实验室下拉框数据
	$.ajax({
