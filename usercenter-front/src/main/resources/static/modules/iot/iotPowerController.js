layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function() {
	...
	//向世界问个好
	layer.msg('进入物联设备-电源控制器');

	form.render(null, 'iotpowercontroller');
	...
	//监听行工具事件
	table.on('tool(iotpowercontrollertab)', function(obj) {
		 ...
		 * @date 2020/10/16
		 */
		if(obj.event === 'openIns'){
			$.ajax({
				url:iotHost+'/api/agent/insOpen',
				type:'post',
				data:{
					hardwareIp : data
				},
		 ...
		 * @date 2020/10/16
		 */
		if(obj.event === 'closeIns'){
			$.ajax({
				url:iotHost+'/api/agent/insClose',
				type:'post',
				data:{
					hardwareIp : data
				},
		...
		if(obj.event === 'connect') {
			$.ajax({
				url:iotHost+'/api/agent/connectIns',
				type:'get',
				data:{
					hardwareIp : data
				},
		...
		if(obj.event === 'logUpload') {
			$.ajax({
				url:iotHost+'/api/agent/logUploadIns',
				type:'get',
				data:{
					hardwareIp : data
				},
		...
		if(obj.event === 'dataDistribute') {
			$.ajax({
				url:iotHost+'/api/agent/dataDistributeIns',
				type:'get',
				data:{
					hardwareIp : data
				},
		...
		if(obj.event === 'regcard') {
			$.ajax({
				url:iotHost+'/api/reservation/regcardIns',
				type:'post',
				data:{
					ip : data
				},
	...
	$.ajax({
		...
		success:function(res){
			...
			form.render('select', "iotpowercontroller");
		},
	})
	//搜索
	var $ = layui.$,
