layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function () {
    ...
    //监听提交
    form.on('submit(newstationbtn)', function (data) {
        ...
        $.ajax({
            url: iotHost + '/api/agent/addIotStation/',
            type: 'post',
            data: {
                hardwareIp,
                sn,
        ...
        parent.layui.table.reload('iotstationinstrumenttab'); //重载表格

    });

    form.render(null, 'newstation');
    //获取实验室下拉框数据
    $.ajax({
