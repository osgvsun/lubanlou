layui.use(['index', 'form', 'laydate', 'upload', 'laypage', 'layer', 'table', 'element'], function() {
	...
	form.render(null, 'startequipmentmaintenancebox');
	...
	//监听提交
	form.on('submit(startequipmentmaintenance)', function(data) {
	...
	form.on('radio(statenormal)', function(data) {
		...
		var abc = data.value;
		if(abc == "否") {
			$(this).siblings(".info_reason").show();
			//$("#nt").val("1");
			//$("#nf").val("");
			//alert($("#nt").val());
		} else {
			$(this).siblings(".info_reason").hide();
			//$("#nt").val("1");
			//$("#nf").val("");
			//alert($("#nf").val());
		}
	...
	//开始设备快速保养信息
	form.val('startequipmentmaintenancebox', {
		"": "" //
	});
