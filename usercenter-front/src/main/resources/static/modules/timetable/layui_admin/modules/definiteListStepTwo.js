...
layui.use(['laypage', 'layer', 'table', 'element', 'tableSelect'], function() {
	...
	getTeacher();
    table.render({
        elem: '#definiteListStepTwo',
        id: 'definiteListStepTwo',
        title: '选择教师',
        cellMinWidth: 100,
        page: true, //开启分页
    ...
    form.on('submit(add_teacher)', function(data){
        ...
        var cache=table.cache['definiteListStepTwo'];
        var t={};
        ...
        if(added.length>0){
        	for(var i=0;i<added.length;i++){
				if(added[i].teacher==teacher&&added[i].identity==t["identity"]){
					console.log('重复')
					return false;
				}
        	}
        ...
        table.reload('definiteListStepTwo',{data:added});
        return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
    });
	//监听行工具事件
	table.on('tool(definiteListStepTwo)', function(obj) {
		var data = obj.data;
		if(obj.event === 'del') {
    ...
    var previousStep = {
        previousStep: function() {
            ...
            window.parent.window.definiteCourse(term,courseNo,teacherlist,0);
        }
    };
    ...
    var nextStep = {
        nextStep: function() {
            ...
            window.parent.window.definiteCourseThree(term,courseNo,teacherlist);
        }
    };
