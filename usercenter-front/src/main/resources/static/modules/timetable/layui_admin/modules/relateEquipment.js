layui.use(['form', 'laypage', 'laydate', 'layer', 'table', 'element', 'formSelects', 'deviceBatchGlobal'], function () {
	...
	var formSelectComponent = {
        ...
        init:function () {
            ...
            document.querySelector("#addRelatedDevice").addEventListener("click", function (evt) {
        ...
        init:function () {
            document.querySelector(this.el).addEventListener("click", function () {
                if(!(saveComponent.parent.addData.length > 0)){
                    ...
                    if(saveComponent.parent.editData){
                ...
                $.ajax({
                    url: layui.setter.base + '../device/saveRelationDevices',
                    method: 'POST',
                    data: saveComponent.parent.getRelateFormData(),
                    traditional: true,
	...
	var tableComonent = {
        ...
        getRelateFormData:function(){
            var formData = {};
            formData.relationName = this.getRelationName();
            formData.relationNumber =this.getRelationNumber();
        ...
        init:function(){
            ...
            if(equipmentNumber||equipmentName){
                $.ajax({
                    url: layui.setter.base + '../device/editRelationDevices',
                    data: {
                        relationName:equipmentName,
                        relationUid:equipmentNumber,
                        page: 1,
                        limit: 1000
                    },
                    traditional: true,
                    ...
                    function deleteData(sourceData,obj){
                        for(var i=0;i<sourceData.length;i++){
                            if(sourceData[i].id === obj.data.id){
                                ...
                                $.ajax({
                                    url:layui.setter.base+'../device/deleteRelationDevice',
                                    data:{
                                        relationNumber:tableComonent.getRelationNumber(),
                                        relationName: tableComonent.getRelationName(),
