...
function newSelfCourse(selfId) {
    var url = location.origin + '/teacherInformationCenter/lims/timetable/self/newSelfCourse?id=-1&term=';
    if(selfId!=null){
        url = location.origin + '/teacherInformationCenter/lims/timetable/self/newSelfCourse?id='+ selfId +'&term=';
    }
    var index = layer.open({
        type: 2,
        title: '教学班管理',
...
function refreshBootstrapTableLayer() {
    ...
    url = zuulUrl + "api/timetable/self/apiSelfCourseListByDateAndPage";
    var params = $("#table_list").bootstrapTable('getOptions')
    params.ajaxOptions.headers.Authorization =getJWTAuthority();
    params.url = url;
    params.silent=true;
...
function getTimetablePlanView(pageNumber) {
    ...
    var url = zuulUrl + "api/timetable/self/apiSelfCourseListByDateAndPage";
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                if (timetableDTOs.length > 0 && row.timetableStyle != 4 && row.timetableStyle != 6) {
                    rt += '<table border="1"><tr><td width="39%">排课日期</td><td width="25%">实验室</td><td' +
                        ' width="15%">授课教师</td><td width="25%">实验项目</td>';
                ...
                for (var i = 0, len = timetableDTOs.length; i < len; i++) {
                    if (timetableDTOs.length > 0&&row.timetableStyle!=4&&row.timetableStyle!=6) {
                        rt += "<tr><td>"+ timetableDTOs[i].timetable + "</td><td>" + timetableDTOs[i].labInfo + "</td><td>" + timetableDTOs[i].teachers + "</td><td>" + timetableDTOs[i].items + "</td>";
                        if (row.timetableStatus==1 && row.baseActionAuthDTO.deleteManageAuth) {
            ...
            formatter: function (value, row, index) {
                ...
                if (row.timetableStatus == 0) {
                    if (row.baseActionAuthDTO.addActionAuth) {
                        ...
                        result += "<a href='javascript:;' class='btn btn-xs green' title='编辑'  onclick=\"newSelfReNoGroupCourseByDateFormat(" + row.termId + ",'" + row.selfId + "')\" ><span class='glyphicon glyphicon-plus'>排课</span></a>&nbsp;";
                        result += "</td></tr></table>";
                    }
                    ...
                    if (row.baseActionAuthDTO.addActionAuth) {
                        if (row.timetableStyle == 5) {
                            result += "<table><tr><td height=\"25px\"><a href='javascript:;' class='btn btn-xs" +
                                " green' title='编辑'  onclick=\"newSelfReNoGroupCourse(" + row.termId + ",'" + row.selfId + "')\" ><span class='glyphicon glyphicon-edit'>调课</span></a>&nbsp;";
                        } else {
                            result += "<table><tr><td height=\"25px\">&nbsp;";
                        }
                        if (row.timetableStyle == 5||row.timetableStyle == 6) {
                            ...
                            result += "<tr><td height=\"25px\"><div style=\"height:20px;\"><a href='javascript:;' class='btn btn-xs green' title='编辑'  onclick=\"deleteTimetable(" + row.termId + ",'" + row.selfId + "')\" ><span class='glyphicon glyphicon-remove'>删除</span></a></div></td></tr>&nbsp;";
                        }
                        result += '</tr></table>';
                    }
            ...
            title: "计划&nbsp;<button class='btn btn-xs green' title='添加'  onclick=\"newSelfCourse()\" ><span" +
            " style='color: white;' class='glyphicon glyphicon-plus'>添加</span></button>",
            width: "10%",
            field: "empty",
            formatter: function (value, row, index) {
                var id = value;
                var result = "【创建人:"+row.selfCreatedBy + "】<br><br>" + row.selfCreatedDate + "<br>";;
    ...
    $("#table_list").bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                var timetableDTOs = row.timetableDTOs;
                if (timetableDTOs.length > 0&&row.timetableStyle!=4&&row.timetableStyle!=6) {
                    rt += '<table border="1"><tr><td width="25%">排课日期</td><td width="50%">实验室</td></tr>';
                }
                for (var i = 0, len = timetableDTOs.length; i < len; i++) {

                    if (timetableDTOs.length > 0&&row.timetableStyle!=4&&row.timetableStyle!=6) {
                        rt += "<tr><td>"+ timetableDTOs[i].timetable + "</td><td>" + timetableDTOs[i].labInfo + "</td></tr>";
                    }
                }
                if (timetableDTOs.length > 0) {
                    rt += '</table>';
...
function selfBatchManage(self_id) {
    var index = layer.open({
        ...
        shadeClose : true,
        area: ['1100px', '500px'],
        content: location.origin + '/teacherInformationCenter/lims/timetable/course/batchManageList?courseNo='+self_id,
        end: function () {
            refreshBootstrapTableLayer();
        }
    });
