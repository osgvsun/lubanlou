layui.use(['form', 'upload'], function() {
	...
	form.render(null, 'addvideo');

	//监听提交
	form.on('submit(addvideobtn)', function(data) {
		var field = data.field; //获取提交的字段
		var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引  

		//提交 Ajax 成功后，关闭当前弹层并重载表格
		//$.ajax({});
		parent.layui.table.reload('addvideo'); //重载表格
	...
	let path= '教学平台/' + coursetitle + '(' + courseId + ')' + '/知识/' +chapName +'/视频';
	console.log(path)
