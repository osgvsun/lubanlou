...
layui.use(['form', 'element', 'layer', 'upload'], function() {
	...
	//监听提交
	form.on('submit(uploadallprojectbtn)', function(data) {
		var field = data.field; //获取提交的字段
		var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引  

		//提交 Ajax 成功后，关闭当前弹层并重载表格
		//$.ajax({});
		parent.layui.table.reload('allproject'); //重载表格
		parent.layer.close(index); //再执行关闭 
	});

	form.render(null, 'uploadallprojectbox');

	//信息
	form.val('uploadallprojectbox', {
		"": ""
	});
	...
	var pblist = $('#pblist'),
		uploadListIns = upload.render({
			elem: '#projectbook',
			url: `${timetableHost}api/operation/importOperationItem?username=${currentUsername}`, //上传接口
			accept: 'file', //所有文件
			multiple: true, //多个上传
