...
layui.use(['laypage', 'layer', 'table', 'element', 'form'], function() {
			...
			success: function (res) {
				if(res.code === 0){
					...
					$.each(res.data,function (index,item) {
						if(item.authorityName === `SUPERADMIN` || item.authName === `DEAN`){
							proNumberEdit = true;
							return false;
						}
					})
	...
	let termImportSelect = new FormSelectComponent({
		elId: 'term',
		dataUrl: `${timetableHost}api/common/select/apiCommonSelectBySelect`,
		type: 'schoolTermList',
		model: 'single'
	...
	let url = `${timetableHost}/api/operation/getAllMyOperationItemList?academyNumber=${academyNumber}`;
	...
	if(hasSuperAdmin)
		url = `${timetableHost}/api/operation/getAllMyOperationItemList`;
	//执行一个表单
	table.render({
		elem: '#allproject',
		url: url, //数据接口
		...
		parseData: function(res){ //res 即为原始返回的数据
			...
			$.each(res.data,function (key,value) {
				let d = value;
				d['deactivateFlag'] = false;
				$.each(myauth.data,function (index,item) {
					if((item.authorityName === `SUPERADMIN` || item.authName === `DEAN`) && !pConfig['showTerm']){
						d['deactivateFlag'] = true;
						return false;
					}
				})
				data.push(d);
