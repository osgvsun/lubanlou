layui.use(['layer', 'table', 'element', 'form'], function() {
	...
	let assigmentType = '';
	if (type === "knowledge" || type == 'null' || type == ''){
		chapterType = 1;
		...
		layer.msg('进入实验报告—小组作业');
	} else if (type === '200' && title_head === 'data') {
		chapterType = 200;
		assigmentType = 'data';
		layer.msg('进入实验数据—小组作业');
	}
	//执行一个表单
	table.render({
		elem: '#homeworklist',
		url: httpBaseUrl + 'api/studentGroupAssignmentListApi', //数据接口
		method: 'GET',
		where: {"chapterType": chapterType, "type": "assignment", "isGroup": 1, "cid": siteId, "username": username},
		title: '列表',
		cellMinWidth: 100,
		page: true, //开启分页			
	...
	table.on('tool(homeworklist)', function(obj) {
		...
		if(obj.event === 'submit') {
			var index = layer.open({
				...
				shade: 0.5,
				maxmin: true,
				content: 'studentSubmitTeamHomework?type=' + type_head + '&title=' + title_head + '&categoryId=' + data.categoryId,
				...
				yes: function(index, layero) {
					...
					var submit = layero.find('iframe').contents().find("#studentsubmitteamhomeworkbtn");
					submit.click();
				},
				btn2: function (index, layero){
					var save = layero.find('iframe').contents().find("#savehomeworkbtn");
					save.click();
				}
			});
			layer.full(index);
