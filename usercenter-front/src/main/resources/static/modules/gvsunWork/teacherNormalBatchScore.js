layui.use(['form', 'transfer'], function() {
	...
	//监听提交
	form.on('submit(teachernormalbatchscorebtn)', function(data) {
		var field = data.field; //获取提交的字段
		var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
		let finalScore = $('input[name=score]').val();  // 成绩
		...
		$.ajax({
			...
			type: 'POST',
			data: {"assignmentId": parentData, "usernames": usernames.toString(), "finalScore": finalScore, "siteId": siteId},
	...
	form.render(null, 'teachernormalbatchscorebox');
	//获取所有到学生名单，用于制作穿梭数据
	let student = [];
	$.ajax({
