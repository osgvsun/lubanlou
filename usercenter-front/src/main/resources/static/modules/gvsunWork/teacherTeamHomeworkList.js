layui.use(['layer', 'table', 'form'], function() {
		...
		newhomework: function() {
			...
			var index = layer.open({
				...
				yes: function(index, layero) {
					...
					var submit = layero.find('iframe').contents().find("#newhomeworkbtn");
					submit.click();
	...
	table.render({
		...
		method: 'GET',
		title: '列表',
		where: {"chapterType": chapterType, "type": assigmentType, "isGroup": 1, "cid": siteId},
		cellMinWidth: 100,
		page: true, //开启分页			
	...
	table.on('tool(homeworklist)', function(obj) {
		...
		if(obj.event === 'correct') {
			var index = layer.open({
				...
				shade: 0.5,
				maxmin: true,
				content: 'teacherteamCorrectList?assignmentId=' + data.id,
		...
		if(obj.event === 'detail') {
			var index = layer.open({
				...
				shade: 0.5,
				maxmin: true,
				content: 'teacherteamHomeworkDetail?type=' + type + '&title=' + title + '&assignmentId=' + data.id,
				success: function (layero, index) {
					layer.setTop(layero); //重点2
				}
			});
