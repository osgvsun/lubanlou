                            ...
                            <div class="layui-timeline-content layui-text">
                                <div class="layui-timeline-title">
                                    <div class="layui-card-body" style="background:#FF00FF; color:white">
                                        ...
                                        <span>${i.count}</span>条数据
                                        <a class="layui-btn layui-btn-primary layui-btn-xs"
                                           target="_blank" href="sj4New?yearCode=${i.yearCode}&system=lims">
                                            详情
                                        </a>
                                    </div>
                                </div>
...
function tableRender() {
    ...
    layui.use(['table'], function () {
        ...
        //监听单元格编辑
        table.on('edit(lay_table_sj4)', function (obj) {
            let index = -1;
            let data = obj.data;
...
//生成报表
function openSj4ColumnLayer() {
    layer.open({
        type: 2,
        title: "生成报表",
        area: ['800px', '600px'],
        content: 'sj4DataSourceList'
    });
}

//导出报表
function reportExportTxt() {
    window.open(datashareHost + "report/exportSJ4Txt?limsAuth="+$.cookie('currentAuthBydatashare')+"&currYear="+localStorage.currYear);
}

//导出报表
function reportExportExcel() {
    window.open(datashareHost + "report/exportSJ4Excel?limsAuth="+$.cookie('currentAuthBydatashare')+"&currYear="+localStorage.currYear);
}
...
layui.use(['upload', 'element','layer'], function () {
    ...
    upload.render({
        elem: '#importSJ4',
        url: datashareHost + 'report/uploadSJ4ByExcel',
...
function delCheckData() {
    let checkStatus = layui.table.checkStatus('lay_table_sj4')
    ...
    if (data.length > 0) {
        layui.layer.confirm('确定删除选中的' + data.length + '条数据？', {
            ...
            $.ajax({
                url: datashareHost + 'delSJ4',
                method: 'post',
                data: {
                    sj4Json: JSON.stringify(data)
                },
                dataType: 'json',
                success: function (res) {
                    tableRender();
...
function modifyData() {
    if (array.length > 0) {
        $.ajax({
            url: datashareHost + 'updateSJ4',
            method: 'post',
            data: {
                sj4Json: JSON.stringify(array)
            },
            dataType: 'json',
            success: function (res) {
                tableRender();
