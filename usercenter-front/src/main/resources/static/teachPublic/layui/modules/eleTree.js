			...
			function() {
				if(options.lazy){
					var str=[
						...
						'lazy-icon" ></i>',
						'{{# }else{ }}',
						'leaf-icon" style="color: transparent;border:none;background:none;" ></i>',
						'{{# } }}'
					].join("");
					return str;
				}
				return ['{{# if(!d[i]["'+options.request.children+'"] || d[i]["'+options.request.children+'"].length===0){ }}',
					'leaf-icon" style="color: transparent;border:none;background:none;"',
					'{{# } }}',
					'"></i>'
				].join("");
			}(),
					...
					if(options.lazy && el.hasClass("lazy-icon")){
						...
						options.load(d,function(getData) {
							...
							if(d[options.request.children] && d[options.request.children].length>0){
								...
								el.css({"color": "transparent","border":"none","background":"none"}).addClass("leaf-icon");
							}
		...
		remove: function(key) {
			...
			this.keySearchToOpera(key,function(data,obj) {
				...
				if(pElem.children(".eleTree-node").length===0){
					pElem.siblings(".eleTree-node-content").children(".eleTree-node-content-icon").children(".layui-icon").css({"color": "transparent","border":"none","background":"none"});
				}
			});
			...
			options.draggable && options.elem.on("mousedown",".eleTree-node-content",function(e) {
				...
				$(document).on("mousemove.docMove",function(e) {
					...
					leaf && groupNode.siblings(".eleTree-node-content")
						...
						.removeClass("icon-rotate").css({"color": "transparent","border":"none","background":"none"});
					// 当前的增加三角
