    ...
    ,ajaxSend = function(){
      ...
      if (that.config.data.isLayui) {
        layui.each(items, function(index, file){
          ...
          formData.append('directoryId', that.config.data.directoryId);
          formData.append('shareState', '私有');
          for (let key in items) {
            formData.append('files', items[key]);
          }
          let ALLeIndex = new Array();
          layui.each(items, function (e, a) {
            ALLeIndex.push(e);
          });
        resourceContainer.uploadFile({
          ...
          afterUploadSuccess: function (data) {
            ...
            successful++, done(ALLeIndex, i), allDone();
          }, uploadFail: function (reason) {
            aborted++, that.msg("请求上传接口出现异常"), error(files), allDone();
          },
          progress: function (res) {
            var percent = Math.floor((res.loaded/res.total) * 100); //百分比
            layui.each(items, function (e, a) {
              options.progress(percent, (options.item ? options.item[0] : options.elem[0]), res, e)
            })
          }
        });
      }
