...
function expectTraining(insUid) {
    ...
    $.ajax({
        ...
        success:function(data){//AJAX查询成功
               // alert(data);
            $('#expectDiv').fadeOut(2000);
            $('#expectTime').text(trainingTime);
            $('#resultExpectDiv').fadeIn(5000);
    ...
    $.ajax({
        ...
        success:function(data){//AJAX查询成功
            if(data=="isBlack") {
                ...
                alert("已成功加入到培训中！");
                window.location.href = "../config/viewInstrumentTrainingForApp?currpage="+currpage+"&insUid="+insUid;
            }
        }
    })
}