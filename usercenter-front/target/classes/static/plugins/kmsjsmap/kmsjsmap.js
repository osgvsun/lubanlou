...
(function($w) {
  ...
  jm.view_provider.prototype = {
    ...
    setZoom: function(zoom) {
      ...
      this.show(true);
      // 根节点居中
      $(window).scrollTop($("jmnode[class=root]").offset().top-$(window).height()/2);
      return true;
    },
    ...
    adaptZoom:function(){
      ...
      this.setZoom(1)
      // $(window).scrollTop($(".jsmind-inner").offset().top-230);
      return true;
    },

    // 展开所有节点
    openNodes:function(){
      return this.toggleNodes($("jmexpander"),"+")
    },

    // 收缩所有节点
    closeNodes:function(){
      return this.toggleNodes($("jmexpander:not(:hidden)"),"-")
    },

    // 节点 具体实现
    toggleNodes:function(nodes,Symbol){
      nodes.each((index,obj)=>{
        if($(obj).text() == Symbol){
          // 就收缩
          $(obj).click();
        }
...
(function($w) {
  ...
  kmsjsmap.init = function(options,callback) {
  ...
  kmsjsmap._load_jsmind = function() {
    ...
    var innerToolBar = $("<div class='lui-jsmind-innerToolBar'><ul><li class='lui_map_icon_zoom_reset mui mui-history_handler_back' title='还原' data-opt='zoomReset' ></li>" +
      ...
      " <li class='lui_map_icon_zoom_out mui mui-adapt' title='适应' data-opt='zoomAdapt'></li>" +
      " <li class='lui_map_icon_zoom_out mui mui-open' title='全部展开' data-opt='openNodes'></li>" +
      " <li class='lui_map_icon_zoom_out mui mui-close' title='全部收缩' data-opt='closeNodes'></li>" +
      " </ul></div>");
  ...
  kmsjsmap.zoomAdapt = function() {
    _jm.view.adaptZoom();
  }

  kmsjsmap.openNodes = function() {
    _jm.view.openNodes();
  }

  kmsjsmap.closeNodes = function() {
    _jm.view.closeNodes();
  }
