     ...
     if (roleObject === 'ROLE_ADMIN') {
        if (item.config.admin == 1&&item.config.category != '系统管理') {
            if(item.config.href.substr(0, 1) == "/"){
                var html = '<li class="layui-nav-item nav-item"> <a  lay-href="' + serverHostFilter + item.config.href + '?currentShowRole=ROLE_STUDENT" lay-direction="2"><i class="layui-icon layui-icon-auz"></i>' +
                    '<cite>' + item.config.name + '</cite></a></li>';
            } else {
                var html = '<li class="layui-nav-item nav-item"> <a  lay-href="' + serverHost + item.config.href + '?currentShowRole=ROLE_STUDENT" lay-direction="2"><i class="layui-icon layui-icon-auz"></i>' +
                    '<cite>' + item.config.name + '</cite></a></li>';
            }
