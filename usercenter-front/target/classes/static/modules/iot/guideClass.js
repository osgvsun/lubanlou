layui.use(['index', 'form', 'laypage', 'laydate', 'layer', 'table', 'element'], function() {
    ...
    table.render({
        ...
        // url: iotHost + "/api/content/ListContentClass", //数据接口
    ...
    //监听行工具事件
    table.on('tool(guideclasstab)', function(obj) {
        ...
        if(obj.event === 'edit') {
            var index = layer.open({
                ...
                shade: 0.5,
                maxmin: true,
                content: 'guideEditClass?id='+obj.data.id,
                zIndex: layer.zIndex //重点1
                ,
                success: function(layero) {
                    layer.setTop(layero); //重点2
