...
$(document).ready(function () {
        ...
        oTableInit.Init = function () {
            $("#table_list").bootstrapTable({
                ...
                columns: [{
                    ...
                    title: "<div style=\"height:20px;\">操作&nbsp;<button class='btn btn-xs green' title='添加'  onclick=\"showdiv()\" ><span style='color: white' class='glyphicon glyphicon-plus'>添加批次</span></button>",
                    width: "25%",
                    field: "empty",
                    formatter: function (value, row, index) {
                        var result = "";
    ...
    //监听选课形式
    $("input[name='ifselect']").click(function(){
        var val=$('input:radio[name="ifselect"]:checked').val();
        var index = [4,5,6]
        if(val==0){
            for (var k in index){
                $("#my_table").find('tr').find('th').eq(index[k]).hide();
                $("#my_table").find('tr').find('td').eq(index[k]).hide();
            }
        }else{
            for (var k in index){
                $("#my_table").find('tr').find('th').eq(index[k]).show();
                $("#my_table").find('tr').find('td').eq(index[k]).show();
            }
        }
    });
    //1.初始化Table
   ...
   //父子表
    ...
    $("#submitButton").on('click', function () {
        ...
        if($("input[name='ifselect']:checked").val()==1){
...
function doSelfReGroupCourse(groupId, batchId,sameNumberId) {
    ...
    var index = layer.open({
        ...
        shadeClose: true,
        area: ['1100px', '500px'],
        content: location.origin + '/teacherInformationCenter/lims/timetable/self/newSelfReGroupTimetableCourse?currpage=1&flag=0&timetableStyle=6&selfId=' + selfId + "&term=" + term + "&groupId=" + groupId
