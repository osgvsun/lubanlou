...
layui.use(['laypage', 'layer', 'table', 'element'], function() {
    ...
    //向世界问个好judgmentNobatchesListStepThree
	// layer.msg('进入排课');
    let cols = [ //表头
        {
            field: 'timetable',
    ...
    //执行一个已安排时间段表单
    table.render({
        elem: '#judgmentNobatchesListStepThree',
        title: '已安排时间段',
        cellMinWidth: 100
        ,page: true //开启分页
        ...
        id: 'judgmentNobatchesListStepThree',
        data: [],
        skin: 'line', //表格风格
        even: true,
        limits: [5, 7, 10, 20],
    ...
    if($('#adjustStatus').val()==1){
        // $('#add_teacher').html('选择时间段<a class="layui-btn layui-btn-xs" onclick="addTeachers()">确定可选择教师</a>');
        $('#fr').prepend('<a class="layui-btn" onclick="addTeachers()">确定可选择教师</a>');
    }
    getPageInfoRedis();
    getCourseInfo();
    getSelectList();
    ...
    function getAlRecords() {
        ...
        if($('#timetableStyle').val() == 3){
            groupId = '';
        }
        $.ajax({
            // url: weekUrl + "?term=16&weekday=-1",
            url: zuulUrl + "api/timetable/common/apiViewTimetableInfo?courseNo="+courseNo+"&groupId="+groupId,
            ...
            success: function (res) {
                // console.log(res);
                table.reload('judgmentNobatchesListStepThree', {
                    data:res.rows
                });
            }
        });
    ...
    var getStudentInfo = {
        getStudentInfo: function() {
            ...
            var termId = $('#termid').val();
            var courseNo = $('#courseNo').val();
            // if($('#groupId').val()!=-1&&$('#groupId').val()!=""){
            if($('#timetableStyle').val() == 4){
                var groupId = $('#groupId').val();
                var url = contextPath + '/lims/timetable/course/getStudentConflictInfo?timetableStyle='+ $('#timetableStyle').val() +'&groupId='+ groupId+'&termId='+termId+'&tags='+tags+'&courseNo='+courseNo;
            }else{
                var url = contextPath + '/lims/timetable/course/getStudentConflictInfo?timetableStyle='+ $('#timetableStyle').val() +'&termId='+termId+'&tags='+tags+'&courseNo='+courseNo
            }
    ...
    //添加可选择教师
    window.addTeachers = function (id) {
        layer.msg('确定可选择教师');
        var index = layer.open({
            type: 2, //此处以iframe举例
            // url: ,
            ...
            shade: 0.5,
            maxmin: true,
            content: location.origin + '/teacherInformationCenter/lims/timetable/course/addTeacherAdjust',
            end: function () {
                var teachers = JSON.parse($('#teachers').val());
                teacherList = [];
                $.each(teachers,function (index,item) {
                    var t = new Object();
                    t['teacher'] = item.cname;
                    t['type'] = '教师';
                    t['username'] = item.username;
                    teacherList.push(t);
                })
                // $(".teacher_select").each(function(i,e){
                //     var str = '';
                //     $.each(teachers, function (index, item) {
                //         str = str + '<option value="' + item.username + '">' + item.cname + '</option>';
                //     });
                //     $(this).html(str);
                // });
                // // getTimetableBatchList();
    ...
    var showIcon = {
        showIcon: function() {
            var str = '<div>教师不冲突:<i class="teacher_pass"></i>教师冲突:<i class="teacher_danger"></i></div>'+
                ...
                '<div>实验室不冲突:<label class="triangle-bottomleft-green"></label>实验室冲突:<label class="triangle-bottomleft-red"></label>实验室/设备预约冲突:<label class="triangle-bottomleft-orange"></label></div></div>'+
    ...
    function showTimesAndSaveRecords(timetableClass) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // $("div.xm-form-select").click(function(){
                ...
                //
                //     $(this).find("dl").css({"min-width":width+"px", top:top-scrollTop+80+"px", left:left+"px"});
                //
                // });
                ...
                $.each(res.data, function (index, item) {
                    ...
                    form.on('submit('+ singleFilter +')', function(data) {
                        ...
                        if($('#adjustStatus').val()==1){
                            JudgeConflictTimeTableVO = recordsObjectAdjust(weeks,classes,weekday,labRoomId,item,teacher,tutor,desks);
                            JudgeConflictTimeTableVOs.push(JudgeConflictTimeTableVO);
                            // console.log(JudgeConflictTimeTableVOs);
                            saveRecordsAdjust(JudgeConflictTimeTableVOs,index,data.form);
                        }else{
    ...
    function recordsObject(weeks,classes,weekday,labRoomId,item,teacher,tutor,desks) {
        ...
        if($('#groupId').val()!=-1&&$('#groupId').val()!=""){
            JudgeConflictTimeTableVO.groupId = $('#groupId').val();
        }
        // JudgeConflictTimeTableVO.groupId = $('#groupselect').val();
        return JudgeConflictTimeTableVO;
    }
    function recordsObjectAdjust(weeks,classes,weekday,labRoomId,item,teacher,tutor,desks) {
        var JudgeConflictTimeTableVO = new Object();
        JudgeConflictTimeTableVO.weeks = weeks.split(",");
        JudgeConflictTimeTableVO.classes = classes.split(",");
        JudgeConflictTimeTableVO.weekday = weekday;
        JudgeConflictTimeTableVO.term = $('#termid').val();
        JudgeConflictTimeTableVO.courseNo = $('#courseNo').val();
        JudgeConflictTimeTableVO.labRoomIds = labRoomId.split(",");
        JudgeConflictTimeTableVO.items = item.split(",");
        JudgeConflictTimeTableVO.tearchs = teacher.split(",");
        JudgeConflictTimeTableVO.tutors = tutor.split(",");
        JudgeConflictTimeTableVO.status = 11;
        JudgeConflictTimeTableVO.selfId = 0;
        JudgeConflictTimeTableVO.createdBy = username;
        JudgeConflictTimeTableVO.timetableId = 0;
        JudgeConflictTimeTableVO.courseDetailNo = $('#courseDetailNo').val();
        JudgeConflictTimeTableVO.adjustStatus = Number($('#adjustStatus').val());
        JudgeConflictTimeTableVO.sameNumberId = $('#sameNumberId').val();
        JudgeConflictTimeTableVO.timetableStyle = $('#timetableStyle').val();
        JudgeConflictTimeTableVO.adjustWeek = $('#week').val();
        JudgeConflictTimeTableVO.groupId = $('#groupId').val();
        JudgeConflictTimeTableVO.station = desks;
        // JudgeConflictTimeTableVO.groupId = $('#groupselect').val();
        return JudgeConflictTimeTableVO;
    }
    function saveRecordsAdjust(JudgeConflictTimeTableVOs,index,form) {
        JudgeConflictTimeTableVOs[0]['submitNum'] = submitNum;
        $.ajax({
            url: zuulUrl + "api/timetable/adjust/apiTimetableAdjustEnd",
            headers: {Authorization: getJWTAuthority()},
            data: JSON.stringify(JudgeConflictTimeTableVOs[0]),
            async: false,
            type: "post",
            contentType: "application/json;charset=UTF-8",
            success: function (res) {
	...
	//监听行工具事件
	table.on('tool(judgmentNobatchesListStepThree)', function(obj) {
		var data = obj.data;
		if(obj.event === 'del') {
    ...
    var previousStep = {
        previousStep: function() {
            ...
            if($('#adjustStatus').val()==1){
                ...
                var groupId = $('#groupId').val();
                window.parent.window.judgmentNoBatchesCourse(sameNumberId,adjustWeek,courseNo, timeStyle, groupId,2);
            }else{
                window.parent.window.judgmentNoBatchesCourse(term,courseNo);
            }
