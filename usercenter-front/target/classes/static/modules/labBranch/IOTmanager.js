layui.use(['laypage', 'layer', 'table', 'element', 'form', 'laydate'], function() {
	...
	//监听行工具事件
	table.on('tool(iotmanager)', function(obj) {
		var data = obj.data;
		//删除
		if(obj.event === 'del') {
			layer.confirm('是否删除？', {
	...
	//头工具栏事件
	table.on('toolbar(iotmanager)', function(obj) {
		...
		switch(obj.event) {
			case 'addadd':
				var index = layer.open({
					...
					shade: 0.5,
					maxmin: true,
					content: 'addIOTmanager?labRoomId=' + labRoomId,
					...
					yes: function(index, layero) {
						...
						var submit = layero.find('iframe').contents().find("#addiotmanagerbtn");
						submit.click();
					}
				});
				layer.full(index);
			...
			case 'import':
				var index = layer.open({
					...
					shade: 0.5,
					maxmin: true,
					content: 'importIOTmanager?labRoomId=' + labRoomId
				});
				//layer.full(index);
				break;
			case 'delete':
				if(data.length == 0) {
					...
					layer.confirm('是否删除？', {
						...
						$.ajax({
							...
							data: {"array": usernameList.toString(), 'roomId': labRoomId, "username": currentUsername},
							dataType: "JSON",
							success: function (res) {
	...
	function authSet() {
		if ((currentauth !== 'LABMANAGER' && currentauth !== 'EXCENTERDIRECTOR' && currentauth !== 'ACADEMYLEVELM' && currentauth !== 'SUPERADMIN') || statusCenter === 'center' || cookie.get('allstatus') == 1) {
			...
			iotmanager.config.cols[0][4].hide = true;
		} else {
			$('.addadd, .import, .delete').css("display", "inline-block");
			iotmanager.config.cols[0][4].hide = false;
		}
	}
	//根据当前权限显示界面
	$(function () {
