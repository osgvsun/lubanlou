layui.config({
...
}).use(['laypage', 'layer', 'table', 'element', 'jquery', 'eleTree', 'code', 'form', 'slider', 'laydate', 'code'], function () {
    ...
    // 实验室点击事件
    window.setLabRoomName = function (name, url, admins, labRoomArea, labRoomCapacity) {
        cookie.set("labRoomName", name);
        cookie.set("admins", admins);
        cookie.set('labRoomArea', labRoomArea)
        cookie.set('labRoomCapacity', labRoomCapacity)
        location.href = url;
    }

    /** tree init **/
    function _domConcat(labObj) {
        let labUrl = location.origin + "/teacherInformationCenter/openReservation/equipmentReserve"
        let configTypeEnum = ["LAB","STA"]
        let configTypeNameEnum = ["空间预约","工位预约"]
        let _url = function(index){
           return `${labUrl}?configType=${configTypeEnum[index]}&labRoomId=${labObj['labRoomId']}`
        }
        let _name = function(index){
            return `【${configTypeNameEnum[index]}】 - ${labObj['labRoomName']}`
        }
        let _admins = function (index) {
            return `${labObj['admins'].join('、') ? labObj['admins'].join('、') : '暂无'}`;
        }
        return `<div class="grid-item">
                    <div class="card_box">
                        ...
                        <div class="card_bottom">
                            <div class="fr">
                                <a 
                                href="javascript:;" onclick="setLabRoomName(\'${_name(0)}\',\'${_url(0)}\',\'${_admins(0)}\',\'${labObj['labRoomArea']}\',\'${labObj['labRoomCapacity'] ? labObj['labRoomCapacity'] : '0'}\')"
                                   target="i" class="layui-btn layui-btn-xs"
                                   lay-tips="空间预约开放设置">空间预约设置</a>
                                <a 
                                href="javascript:;" onclick="setLabRoomName(\'${_name(1)}\',\'${_url(1)}\',\'${_admins(0)}\',\'${labObj['labRoomArea']}\',\'${labObj['labRoomCapacity'] ? labObj['labRoomCapacity'] : '0'}\')"
                                   target="i" class="layui-btn layui-btn-xs"
                                   lay-tips="工位预约开放设置">工位预约设置</a>
                            </div>
                        </div>
                    </div>
                </div>`;
    ...
    // function _domSuccess() {
    //     $('.grid').masonry({
    //         itemSelector: '.grid-item',
    //         percentPosition: true
    //     });
    //     $('.card_box').mouseover(function () {
    ...
    // let params = {
    //     layer, $, eleTree, _domConcat, _domSuccess
    // }
    // labTreeInit(params)
    /** tree over **/
