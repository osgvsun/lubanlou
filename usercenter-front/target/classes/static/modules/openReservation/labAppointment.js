layui.use(['laypage', 'layer', 'table', 'element', 'form', 'laydate', 'formSelects'], function() {
	...
	form.render(null, 'labappointmentbox');
	...
	//多选
	//formSelects.value('time', [2, 3, 4], true); //多个举例
	formSelects.value('time', [1], true); //测试数据
	...
	// 时间间隔分区
	// 预约时间块显示
	setTimeReservation(interval_time, configMachineUid, maxInterval, minInterval, isAcrossDay, '', username, userSchoolAcademy, openScope, academyNumber);
	// 根据实验室显示时间分段
	//根据预约设置不可预约时间段限制
	// 已预约时间

	form.on('submit(labappointmentbtn)', function(data) {
		...
		let obj_all = { "appUser": username, "appUserMail": field.email, "appUserPhone": field.telephone, "appReason": field.reason, "configUid": configUid, "extendsFieldValue": other_arr, "appType": configType, "appPoint": []};
		let submitDate = [];
		let pass = 0;
		let openScopeTwo = 0;
