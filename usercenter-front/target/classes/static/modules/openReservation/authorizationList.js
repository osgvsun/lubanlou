layui.use(['form', 'element', 'laydate', 'laypage', 'table', 'layer'], function() {
	...
	var newauthorizationlist = {
		newauthorizationlist: function() {
			...
			var index = layer.open({
				...
				shade: 0.5,
				maxmin: true,
				content: '../access/newExamFreeList?entityId=' + labRoomId + '&entityType=' + configType + '&flag=1' + '&parentTable=authorizationlist',
	...
	table.render({
		...
		where: {"entityId": labRoomId, "entityType": configType, "accessEntityId": accessEntityId, "flag": 1},
		title: '列表',
		cellMinWidth: 100,
		page: true, //开启分页			
			...
			[ //表头
				{
					...
					sort: true,
					templet: function (d) {
						return d.datetime + '~' + d.endDateTime
					}
				}, {
					fixed: 'right',
					title: '操作',
