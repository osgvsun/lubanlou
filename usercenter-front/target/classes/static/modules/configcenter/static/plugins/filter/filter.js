...
(function ($) {
	...
	$.fn.comboboxfilter.methods = {
		...
		loadData: function (target, data) {
			...
			var listTarget = $(target).find('.list').html('');
			$this.setData(listTarget, options, data[1], target);
		...
		getValue: function(target) {
			...
			$.each(selected, function(i,item) {
				arrayData.push($(item).attr('data-value'));
				arrayText.push($(item).attr('data-text'));
			});
			object['value'] = arrayData.join(",");
			object['text'] = arrayText.join(",");
			return object;
		},
		...
		addSelected: function (pointTarget, target, itemData, options, targetContain) {
			...
			anode.append(inode);
			var level = $(targetContain).attr('id');
			pointTarget.find('.list').append($(`<li data-type="ParentCatelog" class="selected-tag level_${level.replace(/[^0-9]/ig, "")}"></li>`).append(anode));
		},
		//重新计算Value
		reSetValue: function (target) {
			var value = this.getValue(target).value;
