layui.use(['laypage', 'layer', 'table', 'element', 'form', 'laydate', 'eleTree'], function() {
	...
	//监听提交
	form.on('submit(copyfilesbtn)', function(data) {
		var field = data.field; //获取提交的字段
		var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引  

		//提交 Ajax 成功后，关闭当前弹层并重载表格
		//$.ajax({});
		parent.layui.table.reload('copyfilesbox'); //重载表格
	...
	var obj = {
		elem: '.ele1',
		//data: data,
		url: layui.setter.base + 'modules/resourcesCloud/static/json/copyFiles.json',
