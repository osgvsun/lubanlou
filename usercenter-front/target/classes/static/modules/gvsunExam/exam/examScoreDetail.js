...
layui.use(['laypage', 'layer', 'table', 'element', 'form'], function () {
    ...
    let examScore = '';
    $.ajax({
        url: httpBaseUrl + '/views/findTAssignmentDetail',
        type: 'GET',
        async: false,
        data: {"username": username, "assignmentId": assignmentId, "gradingId": gradingId},
        success: function (res) {
            let data = res;
            examTitle = data.title;
            studentName = data.student;
            examScore = data.stuScore;
            $("input[name='title']").val(data.title);
            $("input[name='teacher']").val(data.teacher);
            $("input[name='student']").val(data.student);
            $("input[name='start']").val(data.startDate);
            $("input[name='end']").val(data.endDate);
            $("input[name='score']").val(data.score);
            $("input[name='detail']").val(data.stuScore);
        }
    })
    ...
    //执行一个表单
    var examScoreDetailExcel =  table.render({
        elem: '#examscoredetail',
        url: httpBaseUrl + '/views/findTestDetailApi', //数据接口
        ...
        parseData: function (result) {
            ...
            //     return d.type === 1 || d.type === 2 || d.type === 4;
            // });
            return {
                "code": 0,
                "msg": result.message,
    ...
    var subjectivedetail =  tableSubject.render({
        ...
        parseData: function (result) {
            sessionStorage.setItem('count1', result.count);
            // var filterData = result.data.filter((d) => {
            //     return d.type === 5 || d.type === 8;
            // });
            return {
                "code": 0,
                "msg": result.message,
                    ...
                    templet: function (res) {
                        ...
                        if (b.length != 0){
                            for (var i = 0; i < b.length; i++){
                               if (b[i].answerText != null){
                                   return '我的答案：' + htmlEscape(b[i].answerText)
                               } else {
                                   return '考生未作答';
                               }
                            }
    ...
    var $ = layui.$,
        activeSecond = {
            reload: function () {
                let searchbox = $('#searchObjectivityBox').val();
    ...
    $('#examScoreDetailExcel').on('click', function () {
        let count= sessionStorage.getItem('count');
        if(count>0){
            $.ajax({
                url: httpBaseUrl + '/views/findTestDetailApi',
                data: {'assignmentId': assignmentId,'gradingId':gradingId, 'username': username, 'page': 1,'dictionary': 1, 'limit': count},
                success: function (res) {
                    console.log(res)
                    exportData = res.data;
                    console.log(examScoreDetailExcel.config.id)
                    examScoreDetailExcel.config.title = examTitle + "_" + studentName +"_" + "成绩单" + ":" + "分数：" + examScore;
                    table.exportFile(examScoreDetailExcel.config.id, exportData, 'xls');
                }
            });
        }else {
            layer.msg("暂无答题")
    ...
    $('#objectiveDetailExcel').on('click', function () {
        ...
        if(count1>0){
            $.ajax({
                url: httpBaseUrl + '/views/findTestDetailApi',
                data: {'assignmentId': assignmentId,'gradingId':gradingId, 'username': username, 'page': 1,'dictionary': 2, 'limit': count1},
                success: function (res) {
                    exportData = res.data;
                    subjectivedetail.config.title = examTitle + "_" + studentName +"_" + "成绩单" + ":" + "分数：" + examScore;
                    table.exportFile(subjectivedetail.config.id, exportData, 'xls');
                }
            })
        }else {
            layer.msg("暂无答题")
