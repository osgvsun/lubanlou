...
public class WebLogAspect {
    ...
    public Object monitorElapsedTime(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        parameters.append("]");
        stopWatch.stop();
        logger.info(String.format("方法耗时: %s,数据源：%s,请求路径：%s,请求参数：%s,对象参数：%s,IP：%s",
                stopWatch.getTime() + "毫秒", clientDatabaseContext.getCurrentDataSourceDto().getSchoolName(), requestUrl, parameters, JSONArray.toJSONString(argList), ipAddr));
        return result;
    }
