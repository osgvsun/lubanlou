-- 计算评分项权重
-- 黄浩 2020年8月24日
DROP PROCEDURE IF EXISTS `proc_work_final_weight_calculate_level1`;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `proc_work_final_weight_calculate_level1`(
...
declare weightIdCur cursor for
	...
	where tgo.experiment_title=experimentTitle and tws.`level`=1;
